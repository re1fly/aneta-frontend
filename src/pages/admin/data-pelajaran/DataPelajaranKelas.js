...
function DataPelajaranKelas() {
  ...
  const [isViewDataPelajaranKelas, setIsViewDataPelajaranKelas] =
    useState(true);
          ...
          variables: [
            {
              ...
              value: {
                tbl_induk: "x_academic_class",
                select: [
                  "x_academic_class.id as id_kelas",
                  "x_academic_class.sub_class",
                ],
                paginate: false,
                join: [
  ...
  const data = dataPelajaran.map((data, index) => {
    return {
      no: index + 1,
      id: data.id_subjects,
      ...
      },
      {
        ...
        render: (text, record) => (
          <Space size="middle">
            <EyeOutlined
              style={{ color: "black" }}
              onClick={() => viewDetailPelKelas(record)}
            />
            <EditOutlined
              style={{ color: "blue" }}
              onClick={() => viewEditPelKelas(record)}
            />
            <DeleteOutlined
              style={{ color: "red" }}
        ...
        {channelList.map((value, index) => (
          <div className="col-xl-4 col-lg-6 col-md-6" key={index}>
            <div className="card mb-4 d-block w-100 shadow-xss rounded-lg p-xxl-5 p-4 border-0 text-center">
              ...
              <h4 className="fw-700 font-xs mt-3 text-capitalize">
                {value.title}
              </h4>
  ...
  const FormCreate = () => {
    return (
      <FormAdminDataPelKelas
        ...
        allSubClass={
          subClass.length != 0 ? (
            <option value="all">Semua Sub Kelas</option>
          ) : null
        }
