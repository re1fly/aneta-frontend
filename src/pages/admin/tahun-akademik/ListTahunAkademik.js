...
export default function ListTahunAkademik() {
      ...
      },
      {
        ...
        render: (text, record) => (
          <Space size="middle">
            ...
            <EditOutlined
              style={{ color: "blue" }}
              onClick={() => viewEditTahunAkademik(record)}
            />
            <DeleteOutlined
              style={{ color: "red" }}
              onClick={() => deleteTahunAkademik(record)}
            />
          </Space>
        ),
      },
  ...
  const creatTahunAkademik = (e) => {
    ...
    console.log(data);
    const akademikInsert = `${data.tahun_akademik} / ${
      parseInt(data.tahun_akademik) + 1
    } `;
