...
function DataGuruAdmin() {
      ...
      .then(function (response) {
        ...
        if (resCode == "success") {
          ...
          notification.error({
            message: "Error",
            description:
              "Gagal menambahkan data guru, mohon cek kembali file excel anda.",
            placement: "top",
          });
        }
      });
  ...
  const _modalImportNew = () => {
    Modal.warning({
      title:
        "Jika anda memilih fitur ini, maka semua data guru akan di replace dengan data excel yang anda impor.",
      width: "800px",
      content: (
        <div>
      ...
      },
      {
        ...
        render: (text, record) => (
          <Space size="middle">
            <EyeOutlined
              style={{ color: "black" }}
              onClick={() => viewDetailGuru(record)}
            />
            <EditOutlined
              style={{ color: "blue" }}
              onClick={() => viewEditGuru(record)}
            />
            <DeleteOutlined
              style={{ color: "red" }}
              onClick={() => deleteGuru(record)}
            />
          </Space>
        ),
      },
        ...
        <Card className="card bg-lightblue border-0 mb-4 text-grey-900">
          <div className="row">
            <div className="col-lg-8 col-md-6 my-2">
              ...
              <FilterAcademic
                getYear={(e) => setAcademic(e.target.value)}
