...
export default function JadwalPelajaranAdminDetail() {
  ...
  const idSubClass = path[0];
  const kelas = path[1];
  const subKelas = path[2];
  // const location = useLocation();
  // const dataKelas = location.state
  ...
  const getListJadwalPelajaran = () => {
    axios
      .post(
        ...
        {
          processDefinitionId:
            "getjadwalpelajarandetail:1:69d67f83-7f8b-11ed-bb6a-a2fb3d782380",
        ...
        variables: [
          {
            ...
            value: {
              ...
              select: ["users.name", "x_academic_teachers.id as id_guru"],
              paginate: 1000,
              join: [
                {
                  tbl_join: "users",
      ...
      // },
      {
        ...
        render: (text, record) => (
          <Space size="middle">
            <EyeOutlined
              style={{ color: "black" }}
              onClick={() => viewDetailJadwal(record)}
            />
            <EditOutlined
              style={{ color: "blue" }}
            ...
            <DeleteOutlined
              style={{ color: "red" }}
              onClick={() => deleteJadwalPelajaran(record)}
            />
          </Space>
        ),
      },
